    ...
    async startGeneration() {
        ...
        while (this.isRunning) {
            ...
            try {
                ...
                const result = await this.runAiSolver(config);
                
                /* 4. Win Chance Calculation */
                const totalCombos = result.lastTotalCombinations ? BigInt(result.lastTotalCombinations) : 1n;
                const validSols = BigInt(result.validSolutionsCountBig || result.solutions.length || 0);
                ...
                if (typeof GameState !== 'undefined' && typeof handlePostSolveAnalysis === 'function') {
                    GameState.results.solutions = result.solutions || [];
                    GameState.results.workerStats = result.stats;
                    GameState.results.targetStats = result.targetStats;
                    GameState.results.conditionStats = result.conditionStats;
                    GameState.results.lastTotalCombinations = result.lastTotalCombinations ? BigInt(result.lastTotalCombinations) : 0n;
                    GameState.results.validSolutionsCount = BigInt(result.validSolutionsCountBig || 0);
